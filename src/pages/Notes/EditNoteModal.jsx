...
export default function EditNoteModal({ note, onClose, onUpdate }) {
  const [employeeName, setEmployeeName] = useState(note.employee_name || '');
  const [date, setDate] = useState(note.date ? note.date.split('T')[0] : '');
  ...
  const handleSubmit = async (e) => {
    ...
    if (!noteText.trim())     { setError('Note content is required.'); return; }

    setSaving(true);
    setError('');
    try {
      // Pass id + payload to Notes.jsx which calls the API
