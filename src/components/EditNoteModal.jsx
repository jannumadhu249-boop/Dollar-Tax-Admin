...
export default function EditNoteModal({ note, onClose, onUpdate }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!date || !noteText) {
      setError('Date and note are required.');
      return;
    }
    onUpdate({ ...note, date, note: noteText, status });
    setError('');
  };
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body" style={bodyStyle}>
            ...
            <div className="form-group" style={groupStyle}>
              ...
              <input type="date" className="search-input-box" value={date} onChange={(e) => setDate(e.target.value)} style={inputStyle} />
            </div>
            <div className="form-group" style={groupStyle}>
              <label style={labelStyle}>Notes</label>
              <textarea className="search-input-box" rows={4} value={noteText} onChange={(e) => setNoteText(e.target.value)} style={textareaStyle} />
            </div>
            <div className="form-group" style={groupStyle}>
              <label style={labelStyle}>Status</label>
              <select className="search-input-box" value={status} onChange={(e) => setStatus(e.target.value)} style={inputStyle}>
          ...
          <div className="modal-footer" style={footerStyle}>
            ...
            <button type="submit" className="btn btn-primary" style={primaryBtnStyle}>Save Changes</button>
          </div>
        </form>
      </div>
    </div>
...
// Inline styles (same as CreateNoteModal)
const overlayStyle = {
  position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.4)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000,
};
const dialogStyle = { backgroundColor: '#fff', borderRadius: '8px', width: '420px', maxWidth: '90%', boxShadow: '0 4px 12px rgba(0,0,0,0.15)' };
